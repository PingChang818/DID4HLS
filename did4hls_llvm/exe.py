...
def dse(space, config):
    ...
    for it in range(config["n_it"]):
        ...
        graphs = utils.extract(pragmas, space["benchmark"], "C:/share", "train", True)
        for i in range(len(graphs)):
            ...
            y = torch.tensor([d.y[0], d.y[1]], dtype=torch.float)
            design.append(Data(x=x, edge_index=edge_index.t().contiguous(), edge_attr=edge_attr, y=y, prob=d.prob).to(config["device"]))
        ...
        cvael = CVAE(len_theta).to(config["device"])
        cvael.train()
        optimizer_el = torch.optim.Adam(cvael.parameters(), lr=config["lr_vae"])
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer_el, milestones=[0.8 * config["eps_vae"]], gamma=0.5)
        tqdm_cvael = tqdm(range(config["eps_vae"]))
        min_loss = float("inf")
        for epoch in tqdm_cvael:
            est, mean, log_var = cvael(feat_l, cond)
            loss = criterion(est, feat_l) - config["lambda_e"] * 0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
            optimizer_el.zero_grad()
            loss.backward()
            optimizer_el.step()
            tqdm_cvael.set_postfix({"loss": loss.item(), "lr": optimizer_el.param_groups[0]["lr"]})
            lr_scheduler.step()
            if min_loss > loss.item():
                min_loss = loss.item()
                torch.save(cvael.state_dict(), "save/cvael.pth")
                
        print("min_loss = {:.4e}".format(min_loss))
        cvael.load_state_dict(torch.load("save/cvael.pth"))
        print("training area estimator:")
        cvaea = CVAE(len_theta).to(config["device"])
        cvaea.train()
        optimizer_ea = torch.optim.Adam(cvaea.parameters(), lr=config["lr_vae"])
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer_ea, milestones=[0.8 * config["eps_vae"]], gamma=0.5)
        tqdm_cvaea = tqdm(range(config["eps_vae"]))
        min_loss = float("inf")
        for epoch in tqdm_cvaea:
            est, mean, log_var = cvaea(feat_a, cond)
            loss = criterion(est, feat_a) - config["lambda_e"] * 0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
            optimizer_ea.zero_grad()
            loss.backward()
            optimizer_ea.step()
            tqdm_cvaea.set_postfix({"loss": loss.item() / len(feat_a), "lr": optimizer_ea.param_groups[0]["lr"]})
            lr_scheduler.step()
            if min_loss > loss.item():
                min_loss = loss.item()
                torch.save(cvaea.state_dict(), "save/cvaea.pth")
                
        print("min_loss = {:.4e}".format(min_loss))
        cvaea.load_state_dict(torch.load("save/cvaea.pth"))
        cvael.eval()
        cvaea.eval()
        ...
        for i_p in range(len(lambda_p)):
            ...
            for epoch in tqdm_opt:
                ...
                z = torch.randn(100, 64, device=config["device"])
                z_l = z + cvael.proj(prob_nn)
                z_a = z + cvaea.proj(prob_nn)
                yl = gatl.predict(cvael.decoder(z_l))
                ya = gata.predict(cvaea.decoder(z_a))
