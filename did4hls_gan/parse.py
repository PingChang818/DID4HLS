...
def cdfg(id_str):
    hls_path = "sim/" + id_str + "/solution/.autopilot/db/"
    for file in os.listdir("sim/" + id_str):
        name = file.split(".")
        if len(name) > 1 and name[1] == "cpp":
            benchmark = name[0]
            break
        
    if not os.path.exists("sim/" + id_str + "/solution/solution_data.json"):
        return
    else:
        rpt = json.load(open("sim/" + id_str + "/solution/solution_data.json", "r"))
        lw = rpt["ModuleInfo"]["Metrics"][benchmark]["Latency"]["LatencyWorst"]
        if lw == "":
            return
    ...
    for file in os.listdir(hls_path):
        name = file.split(".")
        if len(name) == 2 and name[0] != benchmark and name[1] == "adb":
            xml.append(name[0])
            
    if not os.path.exists(hls_path + benchmark + ".adb"):
        print("HLS error: " + id_str + "has no " + benchmark + ".adb!")
    else:
        nodes = []
        edges = []
        calls = []
        ...
        for file in xml:
            ...
            for item in tree.getroot()[0][2][4]:
                if item.tag == "item":
                    ...
                    node["edges"] = es
                    nodes_current.append(node)
                    if file != benchmark and node["opcode"] == "br":
                        returns.append(nid)
        ...
        if nid < 0 or eid < 0:
            print("error: cdfg")
            sys.exit()
        pickle.dump([nodes, edges], open("sim/" + id_str + "/cdfg.pkl", 'wb'))
